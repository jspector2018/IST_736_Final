...
#test model accuracy
roc_auc_score(y_test, clf.predict_proba(x_test), multi_class = 'ovr', average='weighted') #[:,1] after (x_test) deleted
#0.9822892491459847
...
score_2 = roc_auc_score(y_test, SVM_Model._predict_proba_lr(x_test), multi_class = 'ovr', average='weighted') #[:,1] after (x_test) deleted
print("SVM score: ", score_2) #0.9823742114142539
SVM_matrix = confusion_matrix(y_test, SVM_Model.predict(x_test))
print("\nThe confusion matrix is:")
print(SVM_matrix)
print("\n\n")

print(SVM_Model.predict(case311_vector))


list1 = np.array(['win', 'run', 'business', 'high', 'say', 'news', 'home', 'diego', 'san', 'job'])
list2 = np.array(['housing', 'trump', 'right', 'game', 'post', 'state', 'california', 'just', 'diego', 'san'])
list3 = np.array(['real', 'filam', 'good', 'come', 'gloria', 'todd', 'mayor', 'new', 'san', 'diego'])
list4 = np.array(['really', 'home', 'live', 'bay', 'like', 'know', 'people', 'california', 'diego', 'san'])
list5 = np.array(['thank', 'wind', 'flu', 'hit', 'year', 'game', 'amp', 'diego', 'san', 'county'])

list1_vect = vectorizer.transform(list1)
list2_vect = vectorizer.transform(list2)
list3_vect = vectorizer.transform(list3)
list4_vect = vectorizer.transform(list4)
list5_vect = vectorizer.transform(list5)

print(clf.predict(list1_vect))
print(clf.predict(list2_vect))
print(clf.predict(list3_vect))
print(clf.predict(list4_vect))
print(clf.predict(list5_vect))

print(SVM_Model.predict(list1_vect))
print(SVM_Model.predict(list2_vect))
print(SVM_Model.predict(list3_vect))
print(SVM_Model.predict(list4_vect))
print(SVM_Model.predict(list5_vect))
